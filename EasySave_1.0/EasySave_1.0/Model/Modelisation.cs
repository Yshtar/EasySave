    ...
    class Modelisation
    {
        ...
        public void CompleteSave(string inputpathsave, string inputDestToSave, bool copyDir, bool verif) //Function for full folder backup
        {
            ...
            foreach (FileInfo file in files)
            {
                ...
                logState.FileName = Path.Combine(inputpathsave, file.Name);
                logState.FileSize = tempPath;
            ...
            logState.TotalSize = TotalSize;
            logState.FileName = null;
            logState.FileSize = null;
        ...
        private void UpdateStatefile()//Function that updates the status file.
        {
            ...
            if (jsonString.Length != 0) //Checking the contents of the json file is empty or not
            {
                ...
                foreach (var obj in list) // Loop to allow filling of the JSON file
                {
                    ...
                    {
                        obj.FileName = this.Log.FileName;
                        obj.FileSize = this.Log.FileSize;
                ...
                this.serializeObj = Newtonsoft.Json.JsonConvert.SerializeObject(stateList.ToArray(), Formatting.Indented) + Environment.NewLine; //Serialization for writing to json file
        ...
        public void UpdateLogFile(string FileName, string Source, string target)//Function to allow modification of the log file
        {
            ...
            string serializeObj = Newtonsoft.Json.JsonConvert.SerializeObject(datalogs, Formatting.Indented) + Environment.NewLine; //Serialization for writing to json file
            System.IO.File.AppendAllText(directory + @"DailyLogs_" + DateTime.Now.ToString("dd-MM-yyyy") + ".json", serializeObj); //Function to write to log file
        ...
        public void AddSave(EasySave.Model.File backup) //Function that creates a backup job
        {
            ...
            this.serializeObj = Newtonsoft.Json.JsonConvert.SerializeObject(backupList.ToArray(), Formatting.Indented) + Environment.NewLine; //Serialization for writing to json file
        ...
        public void LoadSave(string backupname) //Function that allows you to load backup jobs
        {
            ...
            CompleteSave(backup.SourceFolder, backup.TargetFolder, true, false); //Calling the function to run the full backup
            ...
            Console.WriteLine("Saved Successfull !"); //Satisfaction message displa
        }
