    ...
    class Modelisation : IDisposable
    {
        ...
        public void CompleteSave(string inputpathsave, string inputDestToSave, bool copyDir, bool verif) //Function for full folder backup
        {
            ...
            foreach (FileInfo file in files) 
            {
                ...
                if (Extension.prio.IndexOf(Path.GetExtension(Path.Combine(inputpathsave, file.Name))) > -1)
                {
                    TabPrio[i] = file;
                    i++;
                }
            ...
            while(c < TabPrio.Length) 
            {
                if (TabPrio[c] != null)
                {
                    if (uppersize == 0)
                    {
                        if (TabPrio[c].Length > 6000)
                            Interlocked.Increment(ref uppersize);
                        copyingfile = TabPrio[c];
                        string tempPath = Path.Combine(inputDestToSave, copyingfile.Name);
                        CryptOrSave(copyingfile, tempPath);
                        c++;
                        if(uppersize != 0)
                        {
                            do
                            {
                                Interlocked.Decrement(ref uppersize);
                            } while (uppersize != 0);
                        }
                    }
        ...
        private void CryptOrSave(FileInfo file, string endpath)
        {
            ...
            string key = "5";
            if (Extension.crypt.IndexOf(Path.GetExtension(file.Name)) > -1)
            {
                string endfile = Path.Combine(endpath);
        ...
        public void UpdateLogFile(string FileName, string Source, string target)//Function to allow modification of the log file
        {
            ...
            try
            {
                ...
                string Row = "Save";
                XmlDocument doc = Newtonsoft.Json.JsonConvert.DeserializeXmlNode("{" + Row + ":" + serializeObj + "}", "root", true, true);
                doc.Save(dailyxml);
            }
            catch
            {
        ...
        public List<File> NameList()//Function that lets you know the names of the backups.
        {
            ...
            string jsonString = System.IO.File.ReadAllText(backupListFile + @"backupList.json"); //Function to read json file
